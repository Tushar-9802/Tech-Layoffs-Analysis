...
with st.sidebar:
    st.header("Filters")
    selected_country = st.multiselect("Select Country", options=sorted(df['country'].dropna().unique()), default=None)
    selected_industry = st.multiselect("Select Industry", options=sorted(df['industry'].dropna().unique()), default=None)
...
quarterly = quarterly.sort_values('quarter')  # Ensures x-axis order
...
if 'company_size_category' in df.columns:
    ...
    # Create full quarter × size grid to avoid gaps
    ...
    # Smooth Unknown separately (if included)
...
top_6_industries = df_full.groupby('industry')['total_laid_off'].sum().sort_values(ascending=False).head(6).index

# Create complete index of quarters × industries
...
# Drop rows where an industry has 0 layoffs across all quarters (after filtering)
